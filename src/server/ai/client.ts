...
function buildPrompt(input: PolishInput): string {
  ...
  return `You edit BDD Gherkin. Return JSON only. Return the original text verbatim unless there is an unambiguous spelling error, grammar error, clearly inconsistent term, or invalid Gherkin syntax. Repair invalid Gherkin syntax while preserving the scenario's behaviour. Correct malformed Gherkin keywords, scenario headers, step structure, indentation, tables, and Examples blocks when needed. Do not rewrite style, capitalization, acronyms, token names, or domain language. Do not invent steps or behaviour.\n\nSummary context:\n${input.summary}\n\nGherkin:\n${input.gherkin}`;
}
